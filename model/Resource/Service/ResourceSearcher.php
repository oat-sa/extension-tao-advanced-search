...
class ResourceSearcher extends ConfigurableService implements ResultSearcherInterface
{
    ...
    public function search(ResultFilter $filter): ResultUnitCollection
    {
        ...
        $queryBuilder = $search->query()
            ->setLimit($filter->getParameter('max') ?? 100)
            ->setOffset($filter->getParameter('start') ?? 0);

        $criteria = $search->searchType($queryBuilder, $filter->getParameter('classUri'), true);

        $queryBuilder = $queryBuilder->setCriteria($criteria);

        return new ResultUnitCollection(
            new ResultUnit(
                $search->getGateway()->search($queryBuilder)
            )
        );
    }
